package com.oracle.etl.utils.ip;
...
/** */
/**
 * * 用来读取QQwry.dat文件，以根据ip获得好友位置，QQwry.dat的格式是 �?. 文件头，�?8字节 1. 第一个起始IP的绝对偏移， 4字节
 * 2. �?后一个起始IP的绝对偏移， 4字节 �?. "结束地址/国家/区域"记录�? 四字节ip地址后跟的每�?条记录分成两个部�? 1. 国家记录 2.
 * 地区记录 但是地区记录是不�?定有的�?��?�且国家记录和地区记录都有两种形�? 1. �?0结束的字符串 2. 4个字节，�?个字节可能为0x1�?0x2 a.
 * �?0x1时，表示在绝对偏移后还跟�?�?个区域的记录，注意是绝对偏移之后，�?�不是这四个字节之后 b. �?0x2时，表示在绝对偏移后没有区域记录
 * 不管�?0x1还是0x2，后三个字节都是实际国家名的文件内绝对偏�?
 * 如果是地区记录，0x1�?0x2的含义不明，但是如果出现这两个字节，也肯定是跟着3个字节偏移，如果不是 则为0结尾字符�? �?.
 * "起始地址/结束地址偏移"记录�? 1. 每条记录7字节，按照起始地�?从小到大排列 a. 起始IP地址�?4字节 b. 结束ip地址的绝对偏移，3字节
 * 
 * 注意，这个文件里的ip地址和所有的偏移量均采用little-endian格式，�?�java是采�? big-endian格式的，要注意转�?
...
public class IPSeeker {
    ...
    /**
     * 从offset位置读取4个字节为�?个long，因为java为big-endian格式，所以没办法 用了这么�?个函数来做转�?
     * 
     * @param offset
    ...
    /**
     * 从offset位置读取3个字节为�?个long，因为java为big-endian格式，所以没办法 用了这么�?个函数来做转�?
     * 
     * @param offset
     * @return 读取的long值，返回-1表示读取文件失败
     */
    ...
    /**
     * �?条IP范围记录，不仅包括国家和区域，也包括起始IP和结束IP *
     * 
     * 
     * @author 肖斌 liu
     */
    ...
    public static class IPSeekerUtils {
         ...
         * @param encoding
         *            编码方式
         * @return 如果encoding不支持，返回�?个缺省编码的字符�?
         */
    ...
    public List<String> getAllIp() {
		...
		for (long i = ipBegin; i < ipEnd; i += IP_RECORD_LENGTH) {
			try {
				this.readIP(this.readLong3(i + 4), buf); // 读取ip，最终ip放到buf�?
